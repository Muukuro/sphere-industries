			...
			<h1>Press</h1>
				...
				<ul class="articles">
					<?php while($row = mysql_fetch_array($result)) {
						$active = ( isset($_GET['ID']) && $_GET['ID'] == $row['ID'] ) ? 'class="current"' : ''; 
                        ?>
                        
						<li onclick="location.href='press?ID=<?= $row['ID']; ?>';" style="cursor:pointer;" <?= $active; ?>>
                            <?= $row['Date']; ?>
                            <a><?= $row['Title']; ?></a>
                            (<?= $row['Source_abbr']; ?>)
                        </li>
				...
				<a href="archive">View archive &rarr;</a>
		</div>
